...
/**
 * 3、抽取shtd_store库中PART的全量数据进入Hive的ods库中表part。字段排序、类型不
 * 变，同时添加静态分区，分区字段类型为String，且值为当前比赛日的前一天日期（分区字段格
 * 式为yyyyMMdd）。并在hive cli执行show partitions ods.part命令，将结果截图复制粘贴至对
 * 应报告中；
 */

object FullStaticExtractionPart {
  def main(args: Array[String]): Unit = {
    ...
    sparkSession.read.jdbc(mysql, "part", properties).createTempView("mysql_part")
    sparkSession.sql(
      s"""
         |insert overwrite table shtd_store_ods.part
         |partitions(etl_date=20230925)
         |select * from mysql_part
         |""".stripMargin)
    sparkSession.sql(
      s"""
         |show partitions shtd_store_ods.part
         |""".stripMargin).show()
  }
}
