...
/**
 * 2、 抽取shtd_store库中NATION的全量数据进入Hive的ods库中表nation。字段排序、类型
 * 不变，同时添加静态分区，分区字段类型为String，且值为当前比赛日的前一天日期（分区字段
 * 格式为yyyyMMdd）。并在hive cli执行show partitions ods.nation命令，将结果截图复制粘贴
 * 至对应报告中；
 */

object FullStaticExtractionNation {
  def main(args: Array[String]): Unit = {
    ...
    sparkSession.read.jdbc(mysql, "nation", properties).createTempView("mysql_nation")
    sparkSession.sql(
      s"""
         |insert overwrite table shtd_store_ods.nation
         |partitions(etl_date=20230925)
         |select * from mysql_nation
         |""".stripMargin)
    sparkSession.sql(
      s"""
         |show partitions shtd_store_ods.nation
         |""".stripMargin).show()
  }
}
